...
var triangle = new Path({ strokeColor: newcolor, opacity: 1, strokeWidth: 45 });

triangle.dashArray  = [25, 1];


var lscolor = getRandomColor();
// LEFT SPEAKER LINE # 1
//=============================================
var leftPath   = new Path({ strokeColor: 'Black', fillColor: 'Black',  opacity: .1, strokeWidth: 115 });
...
var lscolor2 = getRandomColor();
// LEFT SPEAKER LINE # 2
//=============================================
var leftPath3 = new Path({ strokeColor: 'White',  opacity: 1, strokeWidth: 4000 });
var leftPath4 = new Path({ strokeColor: lscolor2,  opacity: 1, strokeWidth: 4000 });

leftPath3.dashArray = [4, 10], leftPath4.dashArray = [2, 10];


var rscolor = getRandomColor();
// RIGHT SPEAKER LINE # 1
//=============================================
var rightPath  = new Path({ strokeColor: 'Black', opacity: .6, strokeWidth: 115 });
...
var rscolor2 = getRandomColor();

// RIGHT SPEAKER LINE # 2 fillColor: 'White',
//=============================================
var rightPath3 = new Path({ strokeColor: 'Black', fillColor: 'Black', opacity: .1, strokeWidth: 90 });
var rightPath4 = new Path({ strokeColor: rscolor2, opacity: 1, strokeWidth: 80 });
...
var group = new Group({
  children: [triangle, leftPath, leftPath2, rightPath, rightPath2, rightPath3, rightPath4],
  applyMatrix: false,
  strokeJoin: 'round',
  strokeCap: 'round',
...
view.onFrame = function() {
  ...
  for (var i = 1; i <= amount; i++) {
    ...
    leftPath2.segments[i].point = leftPath.segments[i].point;

    leftPath3.segments[i].point = [i * step, -leftBands[i - 1] * scale + 1000];
    leftPath4.segments[i].point = [i * step, -rightBands[i - 1] * scale * (flip ? -1 : 1) + 1000 ];

    rightPath.segments[i].point = [i * step, -rightBands[i - 1] * scale * (flip ? -1 : 1) + 140 ];
    rightPath2.segments[i].point = rightPath.segments[i].point;

    rightPath3.segments[i].point = [i * step, -rightBands[i - 1] * scale * (flip ? -1 : 1) + 60 ];
    rightPath4.segments[i].point = rightPath3.segments[i].point;

    triangle.segments[i].point = [i * step, -leftBands[i - 1] * scale - 70];
  ...
  group2.pivot = [righttPath.position.x, 0];
  group2.position = view.center;
}
...
"21 Holberg Suite.mp3"
  ...
  var audioURL = "classical_audio/"  + thissong;
